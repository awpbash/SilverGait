...
export const FORM_FEEDBACK_EXERCISES = new Set(['chair-stand', 'wall-push', 'heel-raise', 'sit-to-stand-hold']);
...
export function useExerciseFormFeedback(config: ExerciseFormConfig): {
  ...
  // Rep counting via knee flexion cycles
  useEffect(() => {
  ...
  }, [config.kneeAngle, config.isActive]);
